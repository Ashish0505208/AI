...
def a_star(start, goal):
    queue = [(0, start)]
    g_score, f_score, came_from = {start: 0}, {start: tiles_out_of_place(start, goal)}, {start: None}
    while queue:
        ...
        for neighbor in get_neighbors(current):
            ...
            if neighbor not in g_score or tentative_g_score < g_score[neighbor]:
                came_from[neighbor] = current
                g_score[neighbor], f_score[neighbor] = tentative_g_score, f_score_neighbor
                heapq.heappush(queue, (f_score_neighbor, neighbor))
                print("\nState:")
                for row in range(0, 9, 3):
                    print(neighbor[row:row+3])
                print(f"h(n) = {h_score}, g(n) = {tentative_g_score}, f(n) = {f_score_neighbor}")
    return False
def get_puzzle_state(prompt):
    return tuple(int(num) for row in [input(f"{prompt} row {i + 1}: ").split() for i in range(3)] for num in row)
if __name__ == "__main__":
    start_state = get_puzzle_state("Enter the initial state:")
    goal_state = (1, 2, 3, 4, 5, 6, 7, 8, 0)  # Default goal state
    print("\nSolution found!" if a_star(start_state, goal_state) else "\nNo solution exists.")